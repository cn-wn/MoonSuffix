...
suffix path; terminal flags record exact, wildcard, and exception rules. The
trie is private so callers cannot construct an invalid compiled list.
...
5. if no explicit rule matches, the implicit `*` rule contributes one label;
...
Blank lines and `//` comment lines are ignored. `*` is valid only as the complete
leftmost label; `!` is valid only as the first character of an exception rule.
